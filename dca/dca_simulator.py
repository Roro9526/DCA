...
def simulate_dca(
    ...
    # Le cash de dividende reçu à une date est toujours réinvesti intégralement
    # le jour même (aucun report d'un jour sur l'autre), donc le nombre de parts
    # achetées ce jour-là est simplement div_cash_flow / price (forme vectorisée
    # équivalente à une boucle jour par jour).
    if reinvest:
        shares_reinvested = (div_cash_flow / price).where(div_cash_flow > 0, 0.0)
